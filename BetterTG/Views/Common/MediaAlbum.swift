...
//        let fillWidth = false
        ...
        if !itemInfos.isEmpty {
            averageAspectRatio /= CGFloat(itemInfos.count)
        }
        ...
        if !forceCalc {
            if itemInfos.count == 2 {
                if proportions == "ww"
                    ...
                    && itemInfos[1].aspectRatio - itemInfos[0].aspectRatio < 0.2
                {
                    let width = maxSize.width
                    let height = floor(min(
                        width / itemInfos[0].aspectRatio,
                        min(width / itemInfos[1].aspectRatio, (maxSize.height - spacing) / 2.0),
                    ))
                    ...
                    let height = floor(min(
                        width / itemInfos[0].aspectRatio,
                        min(width / itemInfos[1].aspectRatio, maxSize.height),
                    ))
                    ...
                    let secondWidth = floor(min(
                        0.5 * (maxSize.width - spacing),
                        round((maxSize.width - spacing) / itemInfos[0]
                            .aspectRatio / (1.0 / itemInfos[0].aspectRatio + 1.0 / itemInfos[1].aspectRatio)),
                    ))
                    let firstWidth = maxSize.width - secondWidth - spacing
                    let height = floor(min(
                        maxSize.height,
                        round(min(firstWidth / itemInfos[0].aspectRatio, secondWidth / itemInfos[1].aspectRatio)),
                    ))
                    
                    itemInfos[0].layoutFrame = CGRect(x: 0.0, y: 0.0, width: firstWidth, height: height)
                    itemInfos[0].position = [.top, .left, .bottom]
                    
                    itemInfos[1].layoutFrame = CGRect(
                ...
                if proportions.hasPrefix("n") {
                    ...
                    let thirdHeight = min(
                        ...
                        round(
                            itemInfos[1].aspectRatio * (maxSize.width - spacing)
                                / (itemInfos[2].aspectRatio + itemInfos[1].aspectRatio),
                        ),
                    )
...
//                    }
                    ...
                    itemInfos[2].layoutFrame = CGRect(
                        x: leftWidth + spacing,
                    ...
                    let secondHeight = min(
                        ...
                        round(min(width / itemInfos[1].aspectRatio, width / itemInfos[2].aspectRatio)),
                    )
                    itemInfos[1].layoutFrame = CGRect(
                        x: 0.0,
                ...
                if proportions == "wwww" || proportions.hasPrefix("w") {
                    ...
                    var h =
                        round((maxSize.width - 2 * spacing) /
                            (itemInfos[1].aspectRatio + itemInfos[2].aspectRatio + itemInfos[3].aspectRatio))
                    ...
                    var w =
                        round((maxSize.height - 2 * spacing) /
                            (1.0 / itemInfos[1].aspectRatio + 1.0 / itemInfos[2].aspectRatio + 1.0 / itemInfos[3]
                                .aspectRatio))
        ...
        if forceCalc || itemInfos.count >= 5 {
            ...
            var attempts = [LayoutAttempt]()
            func addAttempt(_ lineCounts: [Int], _ heights: [CGFloat], _ attempts: inout [LayoutAttempt]) {
                attempts.append(LayoutAttempt(lineCounts: lineCounts, heights: heights))
            }
            ...
            for firstLine in 1..<croppedRatios.count {
                ...
                addAttempt(
                    [firstLine, croppedRatios.count - firstLine],
                    [
                        multiHeight(Array(croppedRatios[0..<firstLine])),
                        multiHeight(Array(croppedRatios[firstLine..<croppedRatios.count])),
                    ],
                    &attempts,
                )
            }
            ...
            for firstLine in 1..<croppedRatios.count - 1 {
                for secondLine in 1..<croppedRatios.count - firstLine {
                    ...
                    addAttempt(
                        ...
                        [
                            multiHeight(Array(croppedRatios[0..<firstLine])),
                            multiHeight(Array(croppedRatios[firstLine..<croppedRatios.count - thirdLine])),
                            multiHeight(Array(croppedRatios[firstLine + secondLine..<croppedRatios.count])),
                        ],
                        &attempts,
                    )
                }
                    ...
                    for secondLine in 1..<croppedRatios.count - firstLine {
                        for thirdLine in 1..<croppedRatios.count - firstLine - secondLine {
                            ...
                            addAttempt(
                                ...
                                [
                                    multiHeight(Array(croppedRatios[0..<firstLine])),
                                    multiHeight(Array(croppedRatios[firstLine..<croppedRatios
                                            .count - thirdLine - fourthLine])),
                                    multiHeight(Array(croppedRatios[firstLine + secondLine..<croppedRatios
                                            .count - fourthLine])),
                                    multiHeight(Array(croppedRatios[firstLine + secondLine + thirdLine..<croppedRatios
                                            .count])),
                                ],
                                &attempts,
                            )
            ...
            if let optimal {
                for i in 0..<optimal.lineCounts.count {
                    ...
                    var positionFlags: ItemPosition = []
                    if i == 0 {
...
extension MediaAlbum {
    struct ItemPosition: OptionSet {
        ...
        static let top = ItemPosition(rawValue: 1)
        static let bottom = ItemPosition(rawValue: 2)
        static let left = ItemPosition(rawValue: 4)
        static let right = ItemPosition(rawValue: 8)
        static let inside = ItemPosition(rawValue: 16)
        static let unknown = ItemPosition(rawValue: 65536)
